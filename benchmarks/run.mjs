...
/**
 * Returns the size of the `fjm` binary in kilobytes
 *
 * @returns number
 */
async function getFilesize() {
  const fjmBinary = await execa("which", ["fjm"])
  const stat = await fs.stat(fjmBinary.stdout.trim())
...
async function runHyperfine() {
  ...
  await execa(
    ...
    [
      ...
      "--warmup=2",
      ...[
        "--command-name=fjm_basic",
        new URL("./basic/fjm", import.meta.url).pathname,
      ],
    ],
    {
