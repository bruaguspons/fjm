...
use crate::{config::FjmConfig, user_version_reader::UserVersionReader};
...
pub struct Use {
    ...
    /// Print informational output to stderr (used internally by `fjm env`)
    #[clap(skip)]
...
fn should_install_interactively(requested_version: &UserVersion) -> bool {
    ...
    let error_message = format!(
        "fjm can't find an installed JDK version matching {}.",
...
fn warn_if_multishell_path_not_in_path_env_var(
    ...
    if let Some(path_var) = std::env::var_os("PATH") {
        let bin_path = multishell_path.join("bin");
...
pub enum Error {
    ...
    #[error(
        "{}\n{}\n{}",
        "We can't find the necessary environment variables to replace the JDK version.",
        "You should setup your shell profile to evaluate `fjm env`, see https://github.com/bruaguspons/fjm#shell-setup on how to do this",
        "Check out our documentation for more information: https://github.com/bruaguspons/fjm"
    )]
    FjmEnvWasNotSourced,
...
pub enum InferVersionError {
    ...
    Local,
    #[error("Could not find any version to use. Maybe you don't have a default version set?\nTry running `fjm default <VERSION>` to set one,\nor create a .java-version file inside your project to declare a JDK version.")]
    Recursive,
}
