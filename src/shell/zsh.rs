...
impl Shell for Zsh {
    ...
    fn use_on_cd(&self, config: &crate::config::FjmConfig) -> anyhow::Result<String> {
        let version_file_exists_condition = "-f .java-version";
        ...
        Ok(formatdoc!(
            r"
                ...
                add-zsh-hook -D chpwd _fjm_autoload_hook
                add-zsh-hook chpwd _fjm_autoload_hook
            ",
...
mod tests {
    ...
    fn use_on_cd_removes_existing_hook_before_adding() {
        let output = Zsh.use_on_cd(&crate::config::FjmConfig::default()).unwrap();
        assert!(output.contains("add-zsh-hook -D chpwd _fjm_autoload_hook"));
    }
}
