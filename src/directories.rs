...
/// A helper struct for directories in fjm that uses XDG Base Directory Specification
...
impl Directories {
    ...
    pub fn default_base_dir(&self) -> PathBuf {
        ...
        let modern = strategy.data_dir().join("fjm");
        if modern.exists() {
            return modern;
        }

        let legacy = strategy.home_dir().join(".fjm");
        if legacy.exists() {
            return legacy;
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let legacy = basedirs.data_dir().join("fjm");
            if legacy.exists() {
                return legacy;
            }
        }
...
mod tests {
    ...
    #[test]
    fn default_base_dir_uses_fjm_branded_path() {
        let dirs = Directories::default();
        let path = dirs.default_base_dir();
        let path_str = path.to_string_lossy();
        assert!(
            path_str.contains("fjm"),
            "expected fjm-branded path, got: {path_str}"
        );
    }

    #[test]
    fn multishell_storage_uses_fjm_multishells_dirname() {
        let dirs = Directories::default();
        let path = dirs.multishell_storage();
        assert_eq!(path.file_name().unwrap(), "fjm_multishells");
    }
}
