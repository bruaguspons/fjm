...
pub enum UserVersion {
    ...
    #[allow(
        dead_code,
        reason = "kept for future JDK semver-range resolution, see PRD §10"
    )]
...
impl UserVersion {
    ...
    /// Matches this user version against a bare remote-listing major number.
    ///
    /// Remote listings (`remote_node_index::list`) only expose majors, so
    /// minor/patch/range components of the requested version can't be
    /// checked here — only the major is compared.
    pub fn matches_major(&self, major: u32) -> bool {
        match self {
            Self::OnlyMajor(m) | Self::MajorMinor(m, _) => u32::try_from(*m) == Ok(major),
            Self::Full(Version::Semver(v)) => u32::try_from(v.major) == Ok(major),
            Self::Full(
                Version::Bypassed | Version::Alias(_) | Version::Lts(_) | Version::Latest,
            )
            | Self::SemverRange(_) => false,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_legacy_format_propagates_error() {
        let err = UserVersion::from_str("1.8.0_311").unwrap_err();
        assert!(matches!(
            err,
            crate::version::Error::LegacyFormatNotSupported
        ));
    }
