    ...
    const child = execa(shell.binaryName(), args, {
      ...
      env: (() => {
        const newProcessEnv: Record<string, string> = {
          ...this.extraEnvVars,
          ...removeAllFjmEnvVars(process.env),
          PATH: [testBinDir(), fjmTargetDir(), process.env.PATH]
            .filter(Boolean)
            .join(path.delimiter),
...
/**
 * The `FJM_DIR` used by `script()` for the current test. Exposed so tests
 * that need to seed a fake install (see `seed-jdk-install.ts`) can compute
 * the exact same directory before executing a script.
 */
export function fjmDirForCurrentTest(): string {
  return path.join(testTmpDir(), "fjm")
}

export function script(shell: Pick<Shell, "dieOnErrors">): Script {
  const fjmDir = fjmDirForCurrentTest()
  rmSync(join(fjmDir, "aliases"), { recursive: true, force: true })
  return new Script({ fjmDir }, shell.dieOnErrors ? [shell.dieOnErrors()] : [])
}

function removeAllFjmEnvVars(obj: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const result: NodeJS.ProcessEnv = {}
  for (const [key, value] of Object.entries(obj)) {
    if (!key.startsWith("FJM_")) {
      result[key] = value
    }
  }
  return result
...
function fjmTargetDir(): string {
  return path.join(
    process.cwd(),
    "target",
    process.env.FJM_TARGET_NAME ?? "debug"
  )
}
